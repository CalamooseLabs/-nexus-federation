...
import { dirname, fromFileUrl, join, toFileUrl, basename } from "@std/path";
  ...
  public async bundle(options: BundleOptions): Promise<BundleResult> {
    ...
    if (importMap?.imports) {
      for (const [key, value] of Object.entries(importMap.imports)) {
        if (value.startsWith(".") && entryPoint.startsWith(".")) {
          ...
          const absolutePath = join(dirname(fromFileUrl(import.meta.url)), relativePath);
          processedImportMap.imports[key] = toFileUrl(join(absolutePath, value)).href;
        } else {
    ...
    try {
      ...
      for (const chunk of chunkFiles) {
        ...
        const dataUrl = `data:application/javascript;charset=utf-8,${encodedChunk}`;
